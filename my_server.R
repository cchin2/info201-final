...
# server for the shiny app that takes in the user input returns 
# the correct output
my_server <- function(input, output) {
  ...
  output$message1 <- renderText({
    msg <- paste0(
      "These graphs show the percentage share for production of ",
  ...
  output$message2 <- renderText({
    ...
    show_pie <- select(correct_colnames_numeric, select_value1)
    msg1 <- paste0(input$element1, ":")
    msg2 <- paste0("Max = ", max(show_pie), " Min = ", min(show_pie))
    msg2 <- paste(msg1, msg2, sep = "\n")
    msg2
  })
  output$message3 <- renderText({
    if (input$element2 == "Crude Oil") {
      select_value2 <- "Crude_Oil"
    } else {
      select_value2 <- "Natural_Gas"
    }
    show_pie <- select(correct_colnames_numeric, select_value2)
    msg1 <- paste0(input$element2, ":")
    msg2 <- paste0("Max = ", max(show_pie), " Min = ", min(show_pie))
    msg2 <- paste(msg1, msg2, sep = "\n")
    msg2
  })
